...
My previous visual explanation was a spectrum. The far left starts with much unknown. We iterate until there is enough known to deliver a solution, and lifecycle stages represents checkpoints in clarity along the way.
...
The core is understanding the problem, which is defined by customer need and achieved through critical thinking. Each phase outputs some artifact, and all artifacts are a restatement of the problem with additional details we've uncovered. In the middle I state how [each phase refines the problem](../_posts/2021-08-13-Swebok-transform-view.md). 
...
Requirements are not just functional requirements (behavior expectations). They should also include quality measurements like maintainability and defect rates. Quantifying and benchmarking these requirements is a whole topic of it's own.
...
However, we also know that problems change. This is where problem modeling kicks up a notch. A key part of modeling is separating essence from accident. That is, what is fundamental to a problem in all cases versus what is temporary or situational. 

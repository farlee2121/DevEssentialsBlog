...
Regarding storage, I was still testing primarily against a real database using Entity Framework at that time. However, a company I worked with had devised a method of using transaction scopes to isolate tests from each other while still using a full database, even with parallel test execution.
...
The problem is that these functions may want to use each other. A few inter-function dependencies is no issue in a local test class, but it quickly spirals out of control with more functions. It also becomes difficult to find the function you need in the ever-growing list.
...
We're also sharing at a broad enough scale now that we run into issues like different test classes wanting to use different data stores. Static references don't effectively support configuration unless we want to pass it as an argument on every call.
...
```cs
TODO: examples of direct construction issues 
- circular references
- 
```
...
```cs
TODO: example leaning on Bogus
```

## Test Reuse

Now that configuration is easy, DataPrep can support running on different data stores configured in the test constructor. This enables another initiative where I reused test as both unit and integration tests. The other main component to enable such a scenario is proper dependency injection in your system components, and passing a flag to the test suite for which mode to configure. I probably won't blog about these efforts because they are better served by TestApi, which I've already [written about](../posts/2022-05-16-TestApi-and-Test-reuse-in-CSharp.md).
